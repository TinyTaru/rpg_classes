...
public class SpinAttackProcedure {
		...
		if ((entity.getCapability(RpgClassesModVariables.PLAYER_VARIABLES_CAPABILITY, null).orElse(new RpgClassesModVariables.PlayerVariables())).Is_Warrior == true) {
			if (world.isClientSide()) {
				if (entity instanceof AbstractClientPlayer player) {
					...
					if (animation != null) {
						animation.setAnimation(new KeyframeAnimationPlayer(PlayerAnimationRegistry.getAnimation(new ResourceLocation("rpg_classes", "spin_attack"))));
					}
				}
			}
			if (!world.isClientSide()) {
					...
					synchronized (connections) {
						...
						while (iterator.hasNext()) {
							...
							if (!connection.isConnecting() && connection.isConnected())
								RpgClassesMod.PACKET_HANDLER.sendTo(new SetupAnimationsProcedure.RpgClassesModAnimationMessage(Component.literal("spin_attack"), entity.getId(), true), connection, NetworkDirection.PLAY_TO_CLIENT);
						}
					}
				}
			}
